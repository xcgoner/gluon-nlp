...
def train(data_train, data_eval, model, nsp_loss, mlm_loss, vocab_size, ctx):
    ...
    # # test local reduction and cross reduction
    # local_reduction_array = mx.nd.array([float(rank), float(rank), float(rank)]).as_in_context(ctx)
    # mx.nd.waitall()
    # logging.info('local_reduction_array before allreduce: {}'.format(local_reduction_array.asnumpy()))
    # allreduce_(local_reduction_array, average=True,
    ...
    #             cross_only = True)
    # mx.nd.waitall()
    # logging.info('local_reduction_array after cross allreduce: {}'.format(local_reduction_array.asnumpy()))
    # return
    ...
    trainer = FP16DistributedLocalSGDTrainerV2(model.collect_params(), args.optimizer, optim_params, local_sgd=local_sgd)
