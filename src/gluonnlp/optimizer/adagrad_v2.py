...
class AdaGradV2(Optimizer):
    ...
    def __init__(self, eps=1e-7, **kwargs):
        super(AdaGradV2, self).__init__(**kwargs)
        self.float_stable_eps = eps
        # self.history_clip = 0.0
    ...
    def update(self, index, weight, grad, state):
        ...
        if is_sparse:
            ...
            raise NotImplementedError('AdaGradV2 has not been implemented for sparse nd')
        else:
            grad = grad * self.rescale_grad
            if self.clip_gradient is not None:
                grad = clip(grad, -self.clip_gradient, self.clip_gradient)
            # history[:] += square(grad)
            history[:] += clip(square(grad), 0, 1.0)
