...
from local_sgd_trainer_v4 import LocalHVDTrainerV4
...
parser.add_argument('--tgt_max_len', type=int, default=-1, help='Maximum length of the target '
                                                                'sentence, -1 means no clipping')
parser.add_argument('--optimizer', type=str, default='localadamv4', help='optimization algorithm')
...
parser.add_argument('--local_sgd_interval', type=int, default=4,
                    help='synchronization interval')
parser.add_argument('--local_sgd_warmup', type=int, default=800,
                    help='warmup steps for local sgd')
...
def train():
    ...
    beta1=0.9
    beta2=0.98
    local_sgd_warmup = args.local_sgd_warmup // args.local_sgd_interval * args.local_sgd_interval
    trainer = LocalHVDTrainerV4(model.collect_params(), args.optimizer, 
                            {'learning_rate': args.lr, 'beta1': beta1, 'beta2': beta2, 
                             'local_sgd_warmup': local_sgd_warmup, 'epsilon': 1e-9}, 
                            local_sgd_interval=args.local_sgd_interval, 
                            local_sgd_warmup=local_sgd_warmup, beta1=beta1, beta2=beta2)
            ...
            if (batch_id + 1) % (args.log_interval * grad_interval) == 0:
                ...
                if is_first_worker:
                    logging.info('[Epoch {} Batch {}/{}] loss={:.4f}, ppl={:.4f}, '
                                'throughput={:.2f}K wps, wc={:.2f}K, lr={:.4f}'
                                .format(epoch_id, batch_id + 1, len(train_data_loader),
                                        log_avg_loss / args.log_interval,
                                        np.exp(log_avg_loss / args.log_interval),
                                        wps / 1000, log_wc / 1000, new_lr*math.sqrt(args.num_units)*10))
                log_start_time = time.time()
                log_avg_loss = 0
                log_wc = 0
        mx.nd.waitall()
