...
from mxnet.ndarray import multi_mp_sgd_update

__all__ = ['mplamb']

@register
class MPLAMB(Optimizer):
    ...
    accumulated in m and v. In MPLAMB, the weight decay term decoupled from gradient
    based update.
    ...
    *Fixing Weight Decay Regularization in Adam*, where the schedule multiplier and
    ...
    The MPLAMB optimizer uses the same learning rate to apply gradients
    ...
    def __init__(self, learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-6,
                 ...
                 **kwargs):
        super(MPLAMB, self).__init__(learning_rate=learning_rate, **kwargs)
        self.beta1 = beta1
        self.beta2 = beta2
    ...
    def create_state_multi_precision(self, index, weight):
        ...
        if self.multi_precision and weight.dtype == numpy.float16:
            ...
            g = weight.astype(numpy.float32)
            return (self.create_state(index, weight_master_copy), weight_master_copy, g)
    ...
    def _update_impl(self, indices, weight, grad, state, multi_precision=False):
        """update function"""
        try:
            from mxnet.ndarray._internal import _prelamb_update
        except ImportError:
            raise ImportError('Failed to import nd.contrib.lamb_update from MXNet. '
                              'MPLAMB optimizer requires mxnet>=1.5.0b20190220. '
                              'Please upgrade your MXNet version. For example: '
                              'pip install mxnet-cu90 --pre. Otherwise, please consider '
                              'LAMB optimizer with different hyper-parameters.')
        ...
        if not multi_precision:
            mean, var = state
            _lamb_update(weight, grad, mean, var, out=grad,
                         lr=1, wd=wd, eta=lr, **kwargs)
            # we only implement the latest version of LAMB
            r1 = weight.norm()
            r1 = minimum(maximum(r1, self.lower_bound), self.upper_bound)
            r2 = grad.norm()
            r = 1. if r1 == 0. or r2 == 0. else r1 / r2
            lr *= r
            # TODO(xcong) sgd_update
            # weight -= lr * grad
            sgd_update(weight, grad, out=weight, lr=lr, wd=0)
        else:
            try:
                from mxnet.ndarray._internal import _mp_prelamb_update
            except ImportError:
                raise ImportError('Failed to import '
                                  'nd.contrib.mp_lamb_update from MXNet. '
                                  'MPLAMB optimizer requires mxnet>=1.5.0b20190220. '
                                  'Please upgrade your MXNet version. For example: '
                                  'pip install mxnet-cu90 --pre. Otherwise, please consider '
                                  'Adam optimizer with different hyper-parameters.')
            mean, var = state[0]
            # TODO(xcong) state[2] is grad of fp32, make it more efficient
            _mp_prelamb_update(weight, grad, mean, var, state[1], out=state[2],
                            lr=1, wd=wd, eta=lr, **kwargs)
            ...
            mp_sgd_update(weight, g, state[1], out=weight, lr=1, wd=0)