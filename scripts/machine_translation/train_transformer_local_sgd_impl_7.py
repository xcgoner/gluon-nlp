...
from gluonnlp.optimizer.local_sgd_v2 import LocalSGDTrainerV2
...
parser.add_argument('--reset_optimizer', type=str, default='adagrad', help='optimizer for fully sync')
parser.add_argument('--lr_rescale', type=float, default=0.1, help='decay of lr, after switching to fully sync')
    ...
    for epoch_id in range(args.start_epoch, args.epochs):
        ...
        if local_sgd_epochs is not None and epoch_id in local_sgd_epochs:
            ...
            if new_local_sgd <= 1:
                ...
                # reset trainer
                trainer = gluon.Trainer(model.collect_params(), args.reset_optimizer)
