...
watchfile=/homes/cx2/src/localadam/uai2020/results/train_transformer_hvd_16.txt

cat $PBS_NODEFILE | uniq > $PBS_O_WORKDIR/hostfile_16
cd /homes/cx2/src/localadam/uai2020/gluon-nlp/scripts/machine_translation
mpirun -np 16 -machinefile $PBS_O_WORKDIR/hostfile_16 -ppn 2 -genv I_MPI_PIN_DOMAIN auto:compact \
                       python train_transformer_hvd.py --dataset WMT2014BPE \
                       --src_lang en --tgt_lang de --batch_size 10800 \
                       --optimizer adam --num_accumulated 2 --lr 2.0 --warmup_steps 4000 \
